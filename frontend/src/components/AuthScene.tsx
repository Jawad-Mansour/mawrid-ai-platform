...
// Layer:   Component
// Purpose: A premium but STABLE backdrop for login/signup — a global import/supply network
//          (tilted orbital rings + travelling shipment nodes) feeding an AI core, a gently
//          twinkling starfield, and slowly tumbling glass "package" cubes. No mouse-parallax
//          (the layers stay put); everything colours off rgb(var(--accent)); respects
...
export function AuthScene() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      {/* soft accent aura (fixed) */}
      <motion.div className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.4 }}>
        <div className="absolute -left-40 top-0 h-96 w-96 rounded-full blur-3xl" style={{ background: A(0.12) }} />
        <div className="absolute -right-32 bottom-0 h-96 w-96 rounded-full blur-3xl" style={{ background: A(0.1) }} />
        <motion.div className="absolute left-1/2 top-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl" style={{ background: A(0.16) }}
          animate={reduce ? {} : { scale: [1, 1.12, 1], opacity: [0.65, 0.9, 0.65] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }} />
      </motion.div>

      {/* twinkling starfield — twinkle in place, no positional drift */}
      <div className="absolute inset-0 hidden md:block">
        {stars.map((s) => (
          <motion.span key={s.id} className="absolute rounded-full"
            style={{ left: `${s.left}%`, top: `${s.top}%`, width: s.size, height: s.size, background: A(0.8) }}
            animate={reduce ? {} : { opacity: [0.12, 0.65, 0.12] }}
            transition={{ duration: s.dur, repeat: Infinity, delay: s.delay, ease: "easeInOut" }} />
        ))}
      </div>

      {/* the orbital network (fixed centre) */}
      <motion.div className="absolute left-1/2 top-1/2 hidden h-0 w-0 md:block" style={{ perspective: 1100 }}
        ...
        <motion.div className="absolute left-1/2 top-1/2 h-24 w-24 -translate-x-1/2 -translate-y-1/2 rounded-full"
          ...
          animate={reduce ? {} : { scale: [1, 1.1, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} />
      </motion.div>

      {/* tumbling glass package cubes (fixed positions, in-place rotation) */}
      <div className="absolute inset-0 hidden lg:block">
        <Cube size={88} dur={30} className="absolute left-[8%] top-[20%]" style={{ opacity: 0.5 }} />
        <Cube size={56} dur={22} className="absolute left-[16%] bottom-[16%]" style={{ opacity: 0.38 }} />
        <Cube size={72} dur={26} className="absolute right-[10%] top-[24%]" style={{ opacity: 0.46 }} />
        <Cube size={48} dur={18} className="absolute right-[18%] bottom-[20%]" style={{ opacity: 0.32 }} />
      </div>
    </div>
  );
}
