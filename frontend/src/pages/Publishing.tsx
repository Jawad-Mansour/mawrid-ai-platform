...
function PublishRow({ p, onDone }: { p: Product; onDone: () => void }) {
  ...
  const publish = useMutation({
    mutationFn: () => apiPost(`/procurement/products/${p.product_id}/publish`, {
      retail_price: Number(price), storefront_qty: Number(qty || 0),
    }),
  ...
  return (
    <tr className="table-row">
      ...
      <td className="px-3 text-ink-soft">{p.qty_in_stock ?? "—"}</td>
      <td className="px-2">
        <div className="relative">
          <Tag className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-ink-faint" />
          <input className="input w-28 pl-7 !py-1.5" placeholder="Price" value={price} onChange={(e) => setPrice(e.target.value)} />
        </div>
      </td>
      <td className="px-2"><input className="input w-20 !py-1.5 text-center" placeholder="Qty" value={qty} onChange={(e) => setQty(e.target.value)} /></td>
      <td className="px-3 text-right">
        {published ? (
          <button className="btn-ghost !py-1.5" onClick={() => unpublish.mutate()}><EyeOff className="h-3.5 w-3.5" /> Unpublish</button>
        ) : (
          <button className="btn-emerald !py-1.5" disabled={!price} onClick={() => publish.mutate()}><Store className="h-3.5 w-3.5" /> Publish</button>
        )}
      </td>
    </tr>
  );
...
export function Publishing() {
  ...
  const all = useMemo(() => asList(q.data).filter((p) => p.enrichment_status === "enriched"), [q.data]);
  ...
  return (
    <div className="space-y-6">
      <SectionTitle title="Storefront Publishing" subtitle="Deliberately select received stock to sell — set the retail price & visible quantity." />
      <Card>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-line text-left text-xs uppercase tracking-wider text-ink-faint">
                ...
                <th className="px-3 font-600">Stock</th><th className="px-2 font-600">Retail price</th>
                <th className="px-2 font-600">Visible qty</th><th className="px-3 font-600"></th>
              </tr></thead>
